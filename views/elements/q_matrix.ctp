...
echo "<h3>" . __("Question", true) . "</h3>";
echo $form->input("QuestionColumn.$i.id");
//echo $form->input("Question.$i.display_order", array("div" => "e_right", "after" => '<span class="add-on">' . __("Order", true) . '</span>', "class" => " span1 s2", "between" => '<div class="input-append">', "label" => false)) . "</div>";
...
if (isset($this->data["QuestionColumn"][$i]["QuestionChoice"])) {
    foreach ($this->data["QuestionColumn"][$i]["QuestionChoice"] as $j => $choice) {
        echo $this->element("q_choice_matrix", array("i" => $i,"j" => $j));
    }
} else {
    echo $this->element("q_choice_matrix", array("i" => $i,"j" => 0));
}
echo '</ul>
 <div class="clear"></div>
